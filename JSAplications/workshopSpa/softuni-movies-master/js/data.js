...
function host(endpoint) {
  return `https://api.backendless.com/0CB26382-E4B2-DD9F-FF20-0D9616C81700/587EDD90-1CCC-4179-8B49-816C7F269F6B/${endpoint}`;
}
...
export async function getMovies(search) {
  ...
  if (!search) {
    ...
    result = (
      await fetch(
        host(
          endpoints.MOVIES + `?where=${escape(`genres LIKE '%${search}%'`)}`
        ),
        {
          headers: {
            "user-token": token,
